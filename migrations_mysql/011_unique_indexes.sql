-- Add UNIQUE indexes for concurrency-safe INSERT IGNORE.
...
DELETE FROM catalogs WHERE id NOT IN (SELECT MIN(id) FROM catalogs GROUP BY path(255));

-- Replace non-unique index with unique one
ALTER TABLE catalogs DROP INDEX IF EXISTS idx_catalogs_path;
CREATE UNIQUE INDEX idx_catalogs_path ON catalogs(path(255));

-- New unique indexes
CREATE UNIQUE INDEX IF NOT EXISTS idx_authors_name_unique ON authors(full_name(255));
CREATE UNIQUE INDEX IF NOT EXISTS idx_series_name_unique ON series(ser_name(255));
